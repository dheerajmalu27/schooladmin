...
export class finalResultReportComponent implements OnInit, AfterViewInit {
  ...
  resultClassDivision(data:any,fileName:any){
    ...
    if(data!=''&& data!=null){
      this.baseservice.get('finalresultbyclass?classId='+data.split('-')[0]+'&divId='+data.split('-')[1]).subscribe((data:any) => {
  ...
  private getFinalResultList() {
    ...
    this.baseservice.get('finalresultclassreportlist').subscribe((data:any) => {
     ...
     this.datatable = $('.m_datatable').mDatatable({
        ...
        columns: [ {
          ...
          template: function (row:any) {
              return '<span class="btn m-btn  m-btn--icon m-badge m-badge--danger m-badge--wide result-button" data-id="' + row.classId+'-'+row.divId+'"  data-filename="' + row.className+'-'+row.divName+'-Final-Result"> Generate </span>';
    ...
    if(formType){
      ...
      this.renderer.listen(this.elRef.nativeElement.querySelector('.m_datatable'), 'click', (e) => {
        ...
        if ((e.target as HTMLElement).classList.contains('result-button')) {
          ...
          const datafilename = (e.target as HTMLElement).getAttribute('data-filename');
