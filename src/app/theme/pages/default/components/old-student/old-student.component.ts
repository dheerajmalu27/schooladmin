...
export class OldStudentComponent implements OnInit, AfterViewInit {
  ...
  public editTemplate(studentData: any) {
    ...
    setTimeout(() => {
      $('.class_select2_drop_down')
        .val(<string>studentData.classId)
        .trigger('change');
      $('.division_select2_drop_down')
        .val(<string>studentData.divId)
        .trigger('change');
      $('.state_select2_drop_down')
        .val(<string>studentData.stateId)
        .trigger('change');
    }, 2000);
  }
  ...
  private getStudentData(Id: any) {
    this.baseservice.get(<string>('student/' + Id)).subscribe(
      (data: any) => {
  ...
  public addStudentSubmitForm(data: any) {
    ...
    if (data.id != '' && data.id != undefined && data.id != null) {
      ...
      this.baseservice.put('student/' + data.id, formData).subscribe(
        (data: any) => {
          this.getStudentList();
          this.listTemplate();
      ...
      console.log(data);
      this.baseservice.post('student', formData).subscribe(
        (data: any) => {
          this.getStudentList();
          this.listTemplate();
