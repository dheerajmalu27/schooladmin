...
@Component({
  ...
  templateUrl: './pending-fees.html',
  encapsulation: ViewEncapsulation.None,
})
export class pendingfeesComponent implements OnInit, AfterViewInit {
  datatable: any; // Change the type to your actual data type
  pendingFeesData: any = null;
  ...
  addTemplate() {
    ...
    this.TitleSet = 'Make Fee Payment';
    this.studentId = null;
  ...
  public calculatepayment(event: any, feeData: any) {
    let totalFeeValue = parseFloat(event.target.value);
    ...
    const remainingAmount = parseFloat(
      (
        parseFloat(this.feesData.totalFee) -
        (totalFeeValue + parseFloat(this.previouslyPaidAmount))
      ).toFixed(2)
    );
  ...
  public AddOldStudentTemplate(pendingFeesData: any) {
    ...
    this.addAdmissionForm.setValue({
      id: null,
      studentId: pendingFeesData.id,
      // firstName: pendingFeesData.firstName,
      // middleName: pendingFeesData.middleName,
      // lastName: pendingFeesData.lastName,
      // // image: pendingFeesData.profileImage,
      // image: '',
      // dob: pendingFeesData.dateOfBirth,
      // classId: pendingFeesData.classId,
      // divId: pendingFeesData.divId,
      // nationality: pendingFeesData.nationality,
      // caste: pendingFeesData.caste,
      // religion: pendingFeesData.religion,
      // address: pendingFeesData.address,
      // bloodGroup: pendingFeesData.bloodGroup,
      // gender: pendingFeesData.gender,
      // motherName: pendingFeesData.motherName,
      // stateId: pendingFeesData.stateId,
      // cityId: pendingFeesData.cityId,
      // motherQual: pendingFeesData.motherQual,
      // parentNumber: pendingFeesData.parentNumber,
      // fatherName: pendingFeesData.fatherName,
      // fatherProf: pendingFeesData.fatherProf,
      // parentNumberSecond: pendingFeesData.parentNumberSecond,
      // prevPercentage: 0,
      totalFee: '',
      tuitionFee: '',
      developmentFee: '',
    ...
    setTimeout(() => {
      $('.class_select2_drop_down')
        .val(<string>pendingFeesData.classId)
        .trigger('change');
      // $(".division_select2_drop_down").val(<string>pendingFeesData.divId).trigger('change');
      $('.state_select2_drop_down')
        .val(<string>pendingFeesData.stateId)
        .trigger('change');
    }, 2000);
  }

  public editTemplate(pendingFeesData: any) {
    ...
    $('#addTemplate').hide();
    $('#addTemplate').show();
    let feesPaymentDetails = JSON.parse(pendingFeesData.feesPaymentDetails);
    this.addAdmissionForm.setValue({
      id: pendingFeesData.id,
      studentId: pendingFeesData.studentId,
      // firstName: pendingFeesData.firstName,
      // middleName: pendingFeesData.middleName,
      // lastName: pendingFeesData.lastName,
      // // image: pendingFeesData.profileImage,
      ...
      // fatherProf: pendingFeesData.fatherProf,
      // parentNumberSecond: pendingFeesData.parentNumberSecond,
      // prevPercentage: pendingFeesData.prevPercentage,
      totalFee:
      ...
      remainingAmount: pendingFeesData.remainingAmount,
      paymentMethod: pendingFeesData.paymentMethod,
      paymentDetail: pendingFeesData.paymentDetail,
    });
  ...
  private getAdmissionList() {
    this.baseservice.get('pendingfees').subscribe(
      (data: any) => {
        this.pendingFeesData = data.pendingfeesstudent;
  ...
  private getPendingStudentList() {
    const formattedData = this.pendingFeesData.map((data: any) => {
      const text = `${data.rollNo}-${data.studentName}-${data.className}-${data.divName}`;
      return { id: data.studentId, text: text };
    });
    console.log(formattedData);
    (<any>$('.student_select2_drop_down')).select2({ data: formattedData });
    $('.student_select2_drop_down').on('change', (event: any) => {
      console.log(event);
      this.onClassChange();
    });
  }
  ...
  onClassChange() {
    const selectedStudentId = $('.student_select2_drop_down').val();

    const selectedStudent = this.pendingFeesData.find(
      (data: any) => data.studentId == selectedStudentId
    );
    let selectedClassId = '';
    let classId = null;
      ...
      this.baseservice.get('feesdatabyclass/' + selectedClassId).subscribe(
        (data: any) => {
          if (typeof data.fees == 'undefined') {
            ...
            this.previouslyPaidAmount =
              parseFloat(data.fees.totalFee) -
              parseFloat(selectedStudent.remainingAmount);
          }
        },
        (error) => {
  ...
  public showtablerecord(data: any) {
    try {
      ...
      this.datatable = $('.m_datatable').mDatatable({
        ...
        data: {
          type: 'local',
          source: data.pendingfeesstudent,
          pageSize: 10,
        },
        ...
        columns: [
            ...
            template: function (row: any) {
              if (
                ...
                return (
                  ...
                  row.id +
                  ')" style="cursor: pointer;"><span class="m-topbar__userpic"><img src="' +
                  environment.apiImageUrl +
                  row.profileImage +
                  '" width="40" height="40" alt="" class="m--img-rounded m--marginless m--img-centered"></span><span (click)="detailProfile(' +
                  row.id +
          ...
          // },
          {
            ...
            template: function (row: any) {
              return (
                '<span class="m-badge m-badge--danger m-badge--wide" style="font-size:14px;"> ' +
                row.remainingAmount +
                ' </span>'
              );
            },
