...
export class divisionAllocationComponent implements OnInit, AfterViewInit {
  ...
  editTemplate(editClassData: any) {
    ...
    this.getSubjectList(
      editClassData.subjectIds,
      editClassData.optionalSubjectIds
    );
    console.log(editClassData);
    $('.edit_total_seats').val(editClassData.totalSeats);
    this.editClassForm = this.fb.group({
      className: editClassData.className,
      totalSeats: editClassData.totalSeats,
    });
  ...
  allocateDivisionByClass(id: any) {
    if (id != '' && id != null) {
      this.baseservice.get('allocatstudentsbyclass/' + id).subscribe(
        (data: any) => {
          this.getSubjectClassTestList();
  ...
  addClassSubmitForm(data: any) {
    ...
    data.optionalSubjectIds = $('.optionalSubjects_select2_drop_down')
      .val()
      .join(',');
    data.className = $('.class_name').val();
    data.totalSeats = $('.total_seats').val();
    ...
    if (data.subjectIds != '' && data.testIds != '' && data.divIds != '') {
      this.baseservice.post('class', data).subscribe(
        (data: any) => {
          this.getSubjectClassTestList();
          // this.addStudentData = data;
          this.listTemplate();
  ...
  editClassSubmitForm(data: any) {
    ...
    data.optionalSubjectIds = $('.edit_optionalSubjects_select2_drop_down')
      .val()
      .join(',');
    data.className = $('.edit_class_name').val();
    data.totalSeats = $('.edit_total_seats').val();
    // console.log($('.division_select2_drop_down').val());
    // console.log($('.test_select2_drop_down').val());
    // console.log($('.subject_select2_drop_down').val());
    if (
      ...
      this.baseservice
        .put('class/' + $('.edit_class_id').val(), data)
        .subscribe(
          (data: any) => {
            this.getSubjectClassTestList();
            // this.addStudentData = data;
  ...
  private getTestList(editTestData: any) {
    this.baseservice.get('testlist').subscribe(
      (data: any) => {
  ...
  private getDivisionList(editValue: any) {
    this.baseservice.get('divisionlist').subscribe(
      (data: any) => {
        ...
        if (editValue) {
          const valuesArray =
            typeof editValue === 'string' ? editValue.split(',') : editValue;
  ...
  private getSubjectList(editSubjectData: any, editOptionalSubjectData: any) {
    this.baseservice.get('subjectlist').subscribe(
      (data: any) => {
        this.subjectData = data.subject;

        // Initialize the primary dropdowns
        $('.subject_select2_drop_down').select2({ data: this.subjectData });
        $('.optionalSubjects_select2_drop_down').select2({
          data: this.subjectData,
        });

        if (editSubjectData) {
          // Convert string values to arrays if needed
          const valuesArraySubject =
            typeof editSubjectData === 'string'
              ? editSubjectData.split(',')
              : editSubjectData;
          const valuesArrayOptional =
            typeof editOptionalSubjectData === 'string'
              ? editOptionalSubjectData.split(',')
              : editOptionalSubjectData;
  ...
  private getSubjectClassTestList() {
    this.baseservice.get('classdetails').subscribe(
      (data: any) => {
        ...
        },
        {
          ...
          template: function (row: any) {
            if (row.admissionCount == row.allocatedCount) {
              ...
              return (
                '<span class="btn m-btn  m-btn--icon m-badge m-badge--danger m-badge--wide allocate-button" data-id="' +
                row.id +
                '"> Allocate </span>'
              );
            }
