...
export class HomeworkComponent implements OnInit, AfterViewInit {
  ...
  $('#m_datepickerSet').on('change', () => {
    ...
    console.log(value);
    this.addHomeworkForm.controls['deadline'].setValue(value);
...
$('.summernote').on('summernote.change', (we: any, contents: any, $editable: any) => {
  ...
  this.addHomeworkForm.controls['description'].setValue(contents); 
...
$('.summernote').on('summernote.change', (we: any, contents: any, $editable: any) => {
  ...
  private editHomeworkData(data:any){
    this.addHomeworkForm.reset();
    let excludeData: any = this.homeworkData.find((homework: any) => homework.id == data);
    this.addHomeworkForm.controls['id'].setValue(excludeData.id);
    this.addHomeworkForm.controls['classId'].setValue(excludeData.classId);
    this.addHomeworkForm.controls['divId'].setValue(excludeData.divId);
    this.addHomeworkForm.controls['subId'].setValue(excludeData.subId);
    this.addHomeworkForm.controls['title'].setValue(excludeData.title);
    this.addHomeworkForm.controls['description'].setValue(excludeData.description);
    this.addHomeworkForm.controls['deadline'].setValue(excludeData.deadline);
  ...
  if(data.classId!='' && data.deadline!='' && data.description!='' && data.divId!=''&& data.subId!='' && data.title!=''){
    if(data.id!=null && data.id!=''){
      this.baseservice.put('homework/'+data.id,data).subscribe((result:any) => { 
  ...
  editHomeworkSubmitForm(data: any){
    this.baseservice.put('homework/'+data.homeworkId,data).subscribe((result:any) => { 
      this.datatable.destroy();
      this.getHomeworkList();
      this.listTemplate();
  ...
  onClassChange() {
    ...
    this.addHomeworkForm.controls['classId'].setValue(selectedClassId);
    ...
    if (selectedClassId) {
      ...
      this.baseservice.get('classsubject/' + selectedClassId).subscribe(
        (data: any) => {
          ...
          console.log(this.addHomeworkForm.controls['subId'].value);
      
          // Set value in Select2 if there is a pre-existing value in the form
          if (this.addHomeworkForm.controls['subId'].value !== null) {
            (<any>$('.subject_select2_drop_down')).val(this.addHomeworkForm.controls['subId'].value).trigger('change');
          }else{
          ...
          $('.subject_select2_drop_down').off('change').on('change', (event: any) => {
            this.addHomeworkForm.controls['subId'].setValue($('.subject_select2_drop_down').val());
          });
        },
        (error) => {
      ...
      this.baseservice.get('classdivision/'+selectedClassId).subscribe(
        (data: any) => {
          ...
          if(this.addHomeworkForm.controls['divId'].value!=null){
            (<any>$('.division_select2_drop_down')).val(this.addHomeworkForm.controls['divId'].value).trigger('change');
          }
          $('.division_select2_drop_down').on('change', (event:any) => {
            this.addHomeworkForm.controls['divId'].setValue($('.division_select2_drop_down').val()); 
          });
        },
        (error) => {
