...
export class SchoolDocsComponent implements OnInit, AfterViewInit {
  ...
  if (!confirmation) {
    ...
    this.baseservice.delete(<string>('schoolotherdocs/' + id)).subscribe((data: any) => {
      this.getSchoolOtherDocslist();
        this.listTemplate();
  ...
  addDocSubmitForm(data: any) {
    ...
    if(data.id==null || data.id==''){
      this.baseservice.post('schoolotherdocs', formData).subscribe((result:any) => {
      ...
      this.baseservice.put('schoolotherdocs/' + data.id, formData).subscribe((result:any) => {
  ...
  private getSchoolOtherDocslist() {
    this.baseservice.get('schoolotherdocs').subscribe((data: any) => {
...
public showtablerecord(data: any) {
  ...
  this.datatable = $('.m_datatable').mDatatable({
    ...
    columns: [ {
      ...
      template: (row: any) => {
        return '<a href="'+environment.apiImageUrl + row.filePath+'" target="_blank"><span class="btn m-btn  m-btn--icon m-badge m-badge--danger m-badge--wide allocate-button" data-id="' + row.id+'"> Download </span></a>';
      ...
      template: function (row:any) {
       return '<span class="btn m-btn m-btn--hover-accent m-btn--icon m-btn--icon-only m-btn--pill"><i class="edit-button la la-edit" data-id="' + row.id + '*'+row.docName+'"></i></span><span class="btn m-btn m-btn--hover-danger m-btn--icon m-btn--icon-only m-btn--pill"><i class="delete-button fa fa-trash-o" data-id="' + row.id + '"></i></span>';
      }
    }]
  });
  ...
  $(this.elRef.nativeElement.querySelector('.m_datatable')).on('click', '.delete-button', (e:any) => {
    e.preventDefault();
    const id = e.target.getAttribute('data-id');
