...
export class AdmissionComponent implements OnInit, AfterViewInit {
  ...
  public calculatepayment(event: any, feeData: any) {
    ...
    const remainingAmount = parseFloat((parseFloat( this.feesData.totalFee) - totalFeeValue).toFixed(2));
    this.addAdmissionForm.get('remainingAmount').setValue(remainingAmount);
    const newTuitionFee = parseFloat(((parseFloat(this.feesData.tuitionFee) / this.feesData.totalFee) * totalFeeValue).toFixed(2));
    this.addAdmissionForm.get('tuitionFee').setValue(newTuitionFee);
    const newExaminationFee = parseFloat(((parseFloat(this.feesData.examinationFee) / this.feesData.totalFee) * totalFeeValue).toFixed(2));
    this.addAdmissionForm.get('examinationFee').setValue(newExaminationFee);
    const newLabFee = parseFloat(((parseFloat(this.feesData.labFee) / this.feesData.totalFee) * totalFeeValue).toFixed(2));
    this.addAdmissionForm.get('labFee').setValue(newLabFee);
    const newDevelopmentFee = parseFloat(((parseFloat(this.feesData.developmentFee) / this.feesData.totalFee) * totalFeeValue).toFixed(2));
    this.addAdmissionForm.get('developmentFee').setValue(newDevelopmentFee);
    const newLibraryFee = parseFloat(((parseFloat(this.feesData.libraryFee) / this.feesData.totalFee) * totalFeeValue).toFixed(2));
    this.addAdmissionForm.get('libraryFee').setValue(newLibraryFee);
    const newOtherFee = parseFloat(((parseFloat(this.feesData.otherFee) / this.feesData.totalFee) * totalFeeValue).toFixed(2));
    this.addAdmissionForm.get('otherFee').setValue(newOtherFee);
    const newSecurityFee = parseFloat(((parseFloat(this.feesData.securityFee) / this.feesData.totalFee) * totalFeeValue).toFixed(2));
    this.addAdmissionForm.get('securityFee').setValue(newSecurityFee);
    const newSportsFee = parseFloat(((parseFloat(this.feesData.sportsFee) / this.feesData.totalFee) * totalFeeValue).toFixed(2));
    this.addAdmissionForm.get('sportsFee').setValue(newSportsFee);
    const newTransportationFee = parseFloat(((parseFloat(this.feesData.transportationFee) / this.feesData.totalFee) * totalFeeValue).toFixed(2));
    this.addAdmissionForm.get('transportationFee').setValue(newTransportationFee);
  ...
  public AddOldStudentTemplate(admissionData: any) {
    ...
    setTimeout(() => {
      $(".class_select2_drop_down").val(<string>admissionData.classId).trigger('change');
      // $(".division_select2_drop_down").val(<string>admissionData.divId).trigger('change');
      $(".state_select2_drop_down").val(<string>admissionData.stateId).trigger('change');
  ...
  public editTemplate(admissionData: any) {
    ...
    this.addAdmissionForm.setValue({
      ...
      totalFee: feesPaymentDetails.totalFee !== undefined ? feesPaymentDetails.totalFee : '',
      tuitionFee: feesPaymentDetails.tuitionFee !== undefined ? feesPaymentDetails.tuitionFee : '',
      developmentFee: feesPaymentDetails.developmentFee !== undefined ? feesPaymentDetails.developmentFee : '',
      examinationFee: feesPaymentDetails.examinationFee !== undefined ? feesPaymentDetails.examinationFee : '',    
      sportsFee: feesPaymentDetails.sportsFee !== undefined ? feesPaymentDetails.sportsFee : '',
      transportationFee: feesPaymentDetails.transportationFee !== undefined ? feesPaymentDetails.transportationFee : '',
      libraryFee: feesPaymentDetails.libraryFee !== undefined ? feesPaymentDetails.libraryFee : '',
      labFee: feesPaymentDetails.labFee !== undefined ? feesPaymentDetails.labFee : '',
      securityFee: feesPaymentDetails.securityFee !== undefined ? feesPaymentDetails.securityFee : '',
      otherFee: feesPaymentDetails.otherFee !== undefined ? feesPaymentDetails.otherFee : '',
      paymentFrequency: feesPaymentDetails.paymentFrequency !== undefined ? feesPaymentDetails.paymentFrequency : '',
      dueDate: feesPaymentDetails.dueDate !== undefined ? feesPaymentDetails.dueDate : '',
      lateFee: feesPaymentDetails.lateFee !== undefined ? feesPaymentDetails.lateFee : '',
      discount: feesPaymentDetails.discount !== undefined ? feesPaymentDetails.discount : '',
  ...
  private getAdmissionList() {
    this.baseservice.get('admission').subscribe((data: any) => {
  ...
  private getAdmissionData(Id: any) {
    this.baseservice.get(<string>('admission/' + Id)).subscribe((data: any) => {
  ...
  private deleteAdmissionData(Id: any) {
    ...
    let isConfirmed = confirm("Are you sure you want to remove this Admission?");

    if (isConfirmed) {
      this.baseservice.delete(<string>('admission/' + Id)).subscribe((data: any) => {
        this.getAdmissionList();
          this.listTemplate();
  ...
  public onSubmit() {
    ...
    if(this.selectedAdmissionType=="new"){
     this.hideModelPopup();
      ...
      this.baseservice.get('student/'+this.selectedStudent).subscribe((data: any) => {
        console.log(data)
        if (data != '') {
          this.hideModelPopup();
  ...
  public onAdmissionTypeChange() {
    ...
    if(this.selectedAdmissionType === 'old'){
      this.baseservice.get(<string>('finalresultstudentlist')).subscribe((data: any) => {
       console.log(data);
  ...
  public addAdmissionSubmitForm(data: any) {
   ...
   data =this.addAdmissionForm.value;
    ...
    if (data.id != '' && data.id != undefined && data.id != null) {
      ...
      this.baseservice.put('admission/' + data.id, formData).subscribe((data: any) => {
        this.getAdmissionList();
        this.listTemplate();
      ...
      console.log(data);
      this.baseservice.post('admission', formData).subscribe((data: any) => {
        this.getAdmissionList();
        this.listTemplate();
